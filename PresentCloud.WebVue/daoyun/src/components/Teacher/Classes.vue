<template>
    ...
    <div style="display: flex;flex-wrap: wrap;"> 
		<div class='content' v-for="item in classesData">
			<div class='header'>
				...
				<router-link :to="{name:'TeacherClass',params: {id:item.id}}">{{item.class_name}}</router-link>
				<!-- <a href="./class"></a> -->
    ...
    <div class='content'>
      ...
      <div class='center' style="text-align:center;border:none;">
          ...
          <p style="font-size: 14px; font-family: PingFangSC-Medium; font-weight: 500; color: rgba(31,32,35,1); line-height: 24px;margin:0;cursor:pointer;" @click="addClassFormVisible = true">创建课程</p>
      </div>
...
export default {
  ...
  methods: {
  	 ...
  	 async handleAddClass() {
        ...
        // 表单提交
        const data = await this.$http.post('/teacherClasses', this.addFormData);
    ...
    async handleDeleteClass(id) {
      	const params = { id: id};
        const data = await this.$http.delete('/teacherClasses', {params});
        if (data.data.meta.status === 200) {
          this.load(localStorage.getItem('uid'));
          this.$message({
            type: 'success',
